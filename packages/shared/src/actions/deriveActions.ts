...
type RulePick = {
  ...
  href: (ctx: { carrierPhone?: string; driverPhone?: string }) => string | undefined;
  ...
  DELIVERY_WINDOW_TEXT: {
    ...
    href: ({ carrierPhone, driverPhone }) =>
      smsHref(
        ...
        "Delivery window is coming up—please confirm ETA. Thanks."
      ),
    dueAtISO: (ex) => ex.dueAtISO,
  },
};

const EXCEPTION_PRECEDENCE: Array<{ codes: ExceptionCode[]; rule: RulePick }> = [
...
export function deriveActions(loads: ActionDeriveLoad[], nowISO: string): BrokerAction[] {
  ...
  for (const l of loads) {
    ...
    // One primary action per load (noise control)
    ...
    actions.push({
      ...
      href: rule.href({ carrierPhone: l.carrierPhone, driverPhone: l.driverPhone }),

      priority: rule.priority,
      dueAtISO: rule.dueAtISO ? rule.dueAtISO(ex) : ex.dueAtISO,
  ...
  // Priority asc, then dueAtISO asc, then createdAtISO
  actions.sort((a, b) => {
